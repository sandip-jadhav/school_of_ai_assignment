...
#   ./run_demo.sh ecommerce    Session 9 E-Commerce skill (Amazon.in keyboard search)
...
query_for() {
  case "$1" in
    ...
    ecommerce)    echo "Find and compare 3 best mechanical wireless keyboards under ₹8,000 on Amazon.in — for each give name, price, rating, and key specs." ;;
    *) return 1 ;;
  esac
}
...
EOF
    ...
    ecommerce)
      cat <<'EOF'
DEMO: ecommerce
  Shape         planner -> ecommerce_browser -> formatter
  Demonstrates  The Session 9 E-Commerce skill on Amazon.in.
                Drives a real Chromium browser: types query in the
                search box, applies a price filter (under ₹8,000),
                applies feature filters (mechanical, wireless), sorts
                by Avg. Customer Review, then extracts a structured
                products list via a second LLM call. No Distiller
                node needed — output.products is already structured.
                Per-turn artifacts land in state/sessions/<sid>/ecommerce/.
                Requires Playwright + V9 gateway on :8109.
EOF
      ;;
  esac
}
...
case "${1:-all}" in
  ...
  wipe)
    ...
    ;;
  hello|shannon|populations|structured|fail|browser|ecommerce)
    precheck; run_one "$1"
    ;;
  all)
    precheck
