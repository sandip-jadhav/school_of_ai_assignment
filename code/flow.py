...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        if resume:
            ...
            graph = Graph()
            _pnid = graph.add_node("planner", inputs=["USER_QUERY"])
            print(f"[graph] add {_pnid} skill=planner deps=-", flush=True)
        ...
        while True:
            ...
            for nid in ready:
                ...
                print(f"[graph] run {nid}", flush=True)
            ...
            for nid, result, prompt in outcomes:
                ...
                _st = "complete" if result.success else "failed"
                graph.mark(nid, _st)
                print(f"[graph] done {nid} status={_st} elapsed={result.elapsed_s:.1f}", flush=True)
                ...
                if result.success:
                    ...
                    _new_nids = graph.extend_from(nid, result, registry=self.registry)
                    for _nn in _new_nids:
                        _nnd = graph.g.nodes[_nn]
                        _preds = [p for p in graph.g.predecessors(_nn) if p.startswith("n:")]
                        print(f"[graph] add {_nn} skill={_nnd['skill']} deps={','.join(_preds) or '-'}", flush=True)
                    ...
                    _rpreds = [p for p in graph.g.predecessors(rec_nid) if p.startswith("n:")]
                    print(f"[graph] add {rec_nid} skill=planner deps={','.join(_rpreds) or '-'}", flush=True)
        ...
        if critic_fail_cap_hit:
            ...
            print(f"\n[flow] WARNING: critic-fail cap hit on "
                  ...
                  f"output too.")
        print(f"\n{'═' * 78}\nFINAL: {(formatter_answer or '')[:4000]}\n{'═' * 78}\n")
