...
def _try_embed(text: str, task_type: str) -> list[float] | None:
    ...
    try:
        ...
        # 503 / connection errors mean the embed model isn't loaded — expected
        # when running without the full gateway stack.  Print once, then stay
        # silent so repeated calls don't flood the output.
        is_unavailable = (
            "503" in str(e) or
            "502" in str(e) or
