...
def _fix_json_newlines(text: str) -> str:
    ...
    LLMs frequently emit unescaped newlines inside strings (e.g. markdown
    tables in a final_answer field) which produces invalid JSON.  This pass
    fixes the encoding without touching the structural whitespace outside
    strings.
    """
    result: list[str] = []
    in_string = False
...
def parse_skill_json(text: str) -> dict:
    ...
    try:
        ...
        # LLMs often emit literal newlines inside JSON string values (e.g.
        # markdown tables in final_answer).  Escape them and retry.
        try:
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "ecommerce_browser":
        # Same bypass pattern as `browser`: EcommerceBrowserSkill owns its
        # own Playwright cascade (no Layer 1 extract, EcommerceA11yDriver →
        # vision fallback) plus a post-navigation product-extraction LLM call.
        # render_prompt is still called above so NodeState.prompt_sent records
        # the skill description for replay, but the skill ignores it.
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
        ...
        sk = EcommerceBrowserSkill(
            artifacts_root=str(ROOT / "state" / "sessions" / session_id / "ecommerce"),
            session=session_id,
        )
        result = await sk.run(node_spec)
        if not result.elapsed_s:
